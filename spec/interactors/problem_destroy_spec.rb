  ...
  context "in unit way" do
    ...
    describe "#execute" do
      ...
      it 'delete all errs associate' do
        Err.should_receive(:destroy_all).with(:id => [err_1.id, err_2.id])
        problem_destroy.execute
      end

      it 'delete all comments associate' do
        Comment.should_receive(:destroy_all).with(:id => [comment_1.id, comment_2.id])
        problem_destroy.execute
      end

      it 'delete all notice of associate to this errs' do
        Notice.should_receive(:destroy_all).with(:err_id => [err_1.id, err_2.id])
        problem_destroy.execute
      end
    end
